...
pub const TONY_TOKEN_KEY: &str = "BChsELmyVyhMEq1FAatNHvQaCaJekiiVrG3VAFWSwoYt";
...
pub mod stake_tony {
    ...
    pub fn init_stake_pool(ctx: Context<InitPool>, pool_bump: u8, reward_duration: u64) -> Result<()>{
        ...
        let cpi_ctx = CpiContext::new(
            ...
            token::Transfer{
                from: ctx.accounts.tony_depositor.to_account_info(),
                to: ctx.accounts.tony_vault.to_account_info(),
                authority: ctx.accounts.tony_deposit_authority.to_account_info(),
            },
        );
...
pub struct InitPool<'info>{
    ...
    #[account(mut, 
    constraint = tony_vault.mint == TONY_TOKEN_KEY.parse::<Pubkey>().unwrap(),
    constraint = tony_vault.owner == pool_signer.key(),
    )]
    tony_vault: Box<Account<'info,TokenAccount>>,
    tony_depositor: Box<Account<'info, TokenAccount>>,
    tony_deposit_authority: Signer<'info>,

    #[account(mut, 
        constraint = staking_vault.mint == TONY_TOKEN_KEY.parse::<Pubkey>().unwrap(),
